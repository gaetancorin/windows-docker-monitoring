...
def update_metrics():
    ...
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Start consommation CPUs par containers")
    ...
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Start consommation mémoires par containers")
    ...
    for c in containers:
        if c.status == "running":
            try:
                ...
                print(c.name, " :", container_memory_used_mb, "MB mémory")
    ...
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Start CPU et mémoire de all docker")
